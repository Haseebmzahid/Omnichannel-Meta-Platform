...
import { InstagramAuthException, InstagramOutsideWindowException } from './instagram.errors';
...
function fakeConversation(overrides: Partial<Conversation> = {}): Conversation {
  return {
    ...
    channelAccountRef: 'ig-account-1',
...
function buildService(opts: {
  ...
  const sendService = { sendText: opts.sendText ?? vi.fn().mockResolvedValue({ externalMessageId: 'ig-mid-ABC' }) } as unknown as InstagramSendService;

  return { service: new InstagramOutboundService(prisma, messageService, sendService), prisma, messageService, sendService, findFirst };
}
...
describe('InstagramOutboundService', () => {
  ...
  it('2. resolves the recipient from the conversation, not a caller-supplied id', async () => {
    const sendText = vi.fn().mockResolvedValue({ externalMessageId: 'ig-mid-ABC' });
    ...
    expect(sendText).toHaveBeenCalledWith(RECIPIENT_IGSID, 'hi');
  });

  it('3. never sends to an Instagram identity belonging to another clinic', async () => {
    ...
    expect(findFirst).toHaveBeenCalledWith(
      expect.objectContaining({ where: expect.objectContaining({ clinicId: OTHER_CLINIC_ID, channelKey: ChannelKey.INSTAGRAM }) }),
    );
    expect(sendService.sendText).not.toHaveBeenCalled();
  });

  it('3b. never sends against a conversation on a different channel (wrong-channel rejected)', async () => {
    const { service, findFirst, sendService } = buildService({ conversation: null });

    await expect(
      service.sendText({ clinicId: CLINIC_ID, conversationId: CONVERSATION_ID, text: 'hi', senderType: 'AI' }),
    ).rejects.toBeInstanceOf(ConversationNotFoundException);

    // The query itself scopes on channelKey: ChannelKey.INSTAGRAM — a
    // WhatsApp (or any non-Instagram) conversation with this id simply
    // never matches, resolving to "not found" the same as a wrong clinic.
    expect(findFirst).toHaveBeenCalledWith(
      expect.objectContaining({ where: expect.objectContaining({ channelKey: ChannelKey.INSTAGRAM }) }),
    );
    expect(sendService.sendText).not.toHaveBeenCalled();
  });
  ...
  it('5. on success, records the external message id and reports delivered', async () => {
    const markOutboundMessageSent = vi.fn().mockResolvedValue(fakeMessage({ deliveryStatus: MessageDeliveryStatus.SENT, externalId: 'ig-mid-ABC' }));
    ...
    expect(markOutboundMessageSent).toHaveBeenCalledWith('message-1', 'ig-mid-ABC');
    expect(result.delivered).toBe(true);
    expect(result.message.externalId).toBe('ig-mid-ABC');
  });

  it('6. a Meta failure is recorded as FAILED and returns a safe result, not a thrown raw error', async () => {
    const sendText = vi.fn().mockRejectedValue(new InstagramOutsideWindowException());
    ...
    expect(markOutboundMessageFailed).toHaveBeenCalledWith(
      'message-1',
      expect.objectContaining({ failureClass: 'window_closed', failureCode: '1545041' }),
    );
  ...
  it('7. an auth failure never leaks the token in the returned failure reason', async () => {
    const sendText = vi.fn().mockRejectedValue(new InstagramAuthException(190));
    const { service } = buildService({ sendText });
  ...
  it('8. a retried call against an already-SENT message never calls Meta again', async () => {
    const alreadySent = fakeMessage({ deliveryStatus: MessageDeliveryStatus.SENT, externalId: 'ig-mid-PRIOR', idempotencyKey: 'retry-key-1' });
  ...
  it('11. first outbound request creates exactly one PENDING-then-persisted message (persistOutboundMessage called once)', async () => {
    const persistOutboundMessage = vi.fn().mockResolvedValue(fakeMessage());
    const { service } = buildService({ persistOutboundMessage });
