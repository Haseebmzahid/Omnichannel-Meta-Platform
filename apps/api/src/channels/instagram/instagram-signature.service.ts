...
// Same generic Graph API webhooks mechanism as WhatsApp's (see
// ../whatsapp/whatsapp-signature.service.ts) — HMAC-SHA256 over the *raw*
// request body, keyed by the app secret, header-formatted as
// "sha256={hex digest}" — confirmed to apply uniformly across webhook
// objects (docs/meta/instagram-messaging.md; ADR-008: "One App Secret
// serves all three webhook objects for HMAC verification, by
// construction"). Implemented independently per channel (own constructor
// arg, own instance) rather than sharing a module with WhatsApp's service,
...
@Injectable()
export class InstagramSignatureService {
