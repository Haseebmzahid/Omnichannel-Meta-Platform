...
// ../whatsapp/whatsapp-outbound-idempotency.integration.spec.ts. Proves the
// outbound idempotency boundary (Message.idempotencyKey) holds through the
// real Messaging Core, not just against a mocked MessageService (already
// covered at the unit level by instagram-outbound.service.spec.ts). Meta
// itself is mocked (InstagramSendService.sendText) — this test never makes
// a real network call.

const RECIPIENT_IGSID = 'igsid-outbound-idempotency-test';
const CHANNEL_ACCOUNT_REF = 'ig-outbound-idempotency-test-account';

describe('Instagram outbound idempotency (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    clinic = await prisma.clinic.create({ data: { name: 'Instagram Outbound Idempotency Test Clinic', timezone: 'UTC' } });
  ...
  it('14. a retried send with the same idempotency key creates exactly one Message and never calls Meta twice', async () => {
    const sendText = vi.fn().mockResolvedValue({ externalMessageId: `ig-mid.${randomUUID()}` });
    const sendService = { sendText } as unknown as InstagramSendService;
    const outboundService = new InstagramOutboundService(prisma, messageService, sendService);
    ...
    expect(afterFirst[0]?.channelKey).toBe(ChannelKey.INSTAGRAM);
  ...
  it('15. reusing the same idempotency key with different text is rejected as a conflict', async () => {
    const sendText = vi.fn().mockResolvedValue({ externalMessageId: `ig-mid.${randomUUID()}` });
    const sendService = { sendText } as unknown as InstagramSendService;
    const outboundService = new InstagramOutboundService(prisma, messageService, sendService);
