...
import type { MessengerMediaRef } from './messenger.normalizer';

// Task 7-9 — the Messenger-specific half of inbound media persistence.
// There is no media-id/auth-fetch step the way WhatsApp has one — Meta's
// own reachable documentation does not confirm whether fetching
// payload.url needs any authentication at all (see docs/meta/
// facebook-messenger.md's "Not established" section); this attempts a
// plain, unauthenticated GET, the best-supported reading of both this
// repo's own already-VERIFIED Instagram note (which shares this exact
// event schema) and third-party implementer reports. If that assumption
// is ever wrong in production, the download fails safely (see below)
// rather than silently mis-authenticating.
//
...
// failed upload.
@Injectable()
export class MessengerMediaIngestService {
  constructor(
  ...
  async ingest(clinicId: string, messageId: string, ref: MessengerMediaRef): Promise<void> {
    try {
      ...
      await uploadAndAttachMedia(this.mediaStorage, this.messageService, {
        ...
        // channel ("No MIME type or file size is provided") — only the
        // downloaded response's own Content-Type header is a real,
        // non-invented source for it.
        mime: downloaded.mime,
      });
    } catch (err) {
      logger.error({ messageId, err: sanitizeError(err) }, 'Messenger: media ingestion failed, message persisted without attachment');
    }
  }
  ...
  private async downloadMedia(url: string): Promise<{ bytes: Buffer; mime?: string } | null> {
    ...
    if (!response.ok) {
      logger.warn({ status: response.status }, 'Messenger: media download failed');
      return null;
    }
