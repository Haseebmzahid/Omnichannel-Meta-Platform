...
const MEDIA_REF: MessengerMediaRef = { url: 'https://scontent.example.test/msgr-image.jpg', attachmentType: AttachmentType.IMAGE };
...
function buildService(fetchImpl: ReturnType<typeof vi.fn>) {
  ...
  const service = new MessengerMediaIngestService(mediaStorage, messageService, fetchImpl as unknown as typeof fetch);
  return { service, mediaStorage, messageService, upload, attachMediaToMessage };
}

describe('MessengerMediaIngestService', () => {
  afterEach(() => {
    vi.clearAllMocks();
  });
