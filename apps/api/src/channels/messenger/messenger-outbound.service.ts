...
import { MessengerSendException } from './messenger.errors';
...
// Messenger — mirrors ../whatsapp/whatsapp-outbound.service.ts and
// ../instagram/instagram-outbound.service.ts exactly: validate -> resolve
// the Messenger recipient from the existing Conversation architecture ->
// persist PENDING through MessageService (Messaging Core) -> call the
// Messenger adapter -> reconcile the persisted row with Meta's outcome ->
...
// own job (never require the caller to manually construct an arbitrary
// PSID).
//
...
// limitation WhatsAppOutboundService/InstagramOutboundService already
// carry — Message.idempotencyKey bounds *this system's* retries, not
// Meta's own at-least-once delivery guarantees on its side; no new
// mechanism is invented to close that gap.
@Injectable()
export class MessengerOutboundService {
  ...
  // satisfy that channel-neutral contract, since SendMessengerTextInput/
  // SendMessengerTextResult already structurally match it.
  readonly channel = ChannelKey.MESSENGER;

  constructor(
    private readonly prisma: PrismaService,
    private readonly messageService: MessageService,
    private readonly sendService: MessengerSendService,
  ) {}

  async sendText(input: SendMessengerTextInput): Promise<SendMessengerTextResult> {
    if (!input.text.trim()) {
      throw new BadRequestException('text must not be empty.');
    }

    // Clinic-scoped by construction: a conversation belonging to another
    // clinic (or a non-Messenger conversation) simply does not match this
    // query and resolves to "not found" — never a cross-clinic send, and
    // never a caller-supplied PSID.
    const conversation = await this.prisma.conversation.findFirst({
      where: { id: input.conversationId, clinicId: input.clinicId, channelKey: ChannelKey.MESSENGER },
    });
    if (!conversation) throw new ConversationNotFoundException(input.conversationId);

    // Conversation.externalThreadKey IS the sender's PSID for a Messenger
    // conversation (see messenger.normalizer.ts's inbound normalization,
    // which sets externalThreadKey = event.sender.id) — no separate
    // ChannelIdentity lookup is needed to recover it.
    const recipientPsid = conversation.externalThreadKey;
    ...
    try {
      const result = await this.sendService.sendText(recipientPsid, input.text);
      ...
      if (err instanceof MessengerSendException) {
      ...
      logger.error({ err: safe }, 'Messenger: outbound send failed unexpectedly');
...
function toFailureRecord(err: unknown): { failureClass: string; failureCode?: string; failureMessage: string } {
  ...
  return { failureClass: 'unknown', failureMessage: 'Messenger send failed unexpectedly.' };
}
