...
import { MessengerAuthException, MessengerOutsideWindowException } from './messenger.errors';
...
function buildService(opts: {
  ...
  const sendService = { sendText: opts.sendText ?? vi.fn().mockResolvedValue({ externalMessageId: 'msgr-mid-ABC' }) } as unknown as MessengerSendService;

  return { service: new MessengerOutboundService(prisma, messageService, sendService), prisma, messageService, sendService, findFirst };
}

describe('MessengerOutboundService', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it('2. resolves the recipient from the conversation, not a caller-supplied id', async () => {
    const sendText = vi.fn().mockResolvedValue({ externalMessageId: 'msgr-mid-ABC' });
    ...
    expect(sendText).toHaveBeenCalledWith(RECIPIENT_PSID, 'hi');
  });

  it('3. never sends to a Messenger identity belonging to another clinic', async () => {
    ...
    expect(findFirst).toHaveBeenCalledWith(
      expect.objectContaining({ where: expect.objectContaining({ clinicId: OTHER_CLINIC_ID, channelKey: ChannelKey.MESSENGER }) }),
    );
    expect(sendService.sendText).not.toHaveBeenCalled();
  });
  ...
  it('3b. never sends against a conversation on a different channel (wrong-channel rejected)', async () => {
    ...
    expect(findFirst).toHaveBeenCalledWith(
      expect.objectContaining({ where: expect.objectContaining({ channelKey: ChannelKey.MESSENGER }) }),
    );
    expect(sendService.sendText).not.toHaveBeenCalled();
  });
  ...
  it('5. on success, records the external message id and reports delivered', async () => {
    const markOutboundMessageSent = vi.fn().mockResolvedValue(fakeMessage({ deliveryStatus: MessageDeliveryStatus.SENT, externalId: 'msgr-mid-ABC' }));
    ...
    expect(markOutboundMessageSent).toHaveBeenCalledWith('message-1', 'msgr-mid-ABC');
    expect(result.delivered).toBe(true);
    expect(result.message.externalId).toBe('msgr-mid-ABC');
  });

  it('6. a Meta failure is recorded as FAILED and returns a safe result, not a thrown raw error', async () => {
    const sendText = vi.fn().mockRejectedValue(new MessengerOutsideWindowException());
    ...
    expect(markOutboundMessageFailed).toHaveBeenCalledWith(
      'message-1',
      expect.objectContaining({ failureClass: 'window_closed', failureCode: '10' }),
    );
  ...
  it('7. an auth failure never leaks the token in the returned failure reason', async () => {
    const sendText = vi.fn().mockRejectedValue(new MessengerAuthException(190));
    const { service } = buildService({ sendText });
  ...
  it('8. a retried call against an already-SENT message never calls Meta again', async () => {
    const alreadySent = fakeMessage({ deliveryStatus: MessageDeliveryStatus.SENT, externalId: 'msgr-mid-PRIOR', idempotencyKey: 'retry-key-1' });
